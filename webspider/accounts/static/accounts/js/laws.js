function googleTranslateElementInit() {
    ...
    const selectedStateFromServer = lawsContainer ? (lawsContainer.dataset.selectedState || '').trim() : '';
    ...
    function filterLaws() {
        ...
        if (!selectedState) {
            for (let i = 0; i < lawItems.length; i++) {
                lawItems[i].style.display = 'none';
            }
            resultsMeta.textContent = 'Select a state or UT to view applicable farmer laws.';
        ...
        let visibleCount = 0;
        for (let i = 0; i < lawItems.length; i++) {
            const item = lawItems[i];
            const haystack = item.dataset.search || '';
            const stateTokens = (item.dataset.states || '').split('|');
            const stateMatch = stateTokens.indexOf('all india') >= 0 || stateTokens.indexOf(selectedState) >= 0;
            const textMatch = !searchTerm || haystack.indexOf(searchTerm) >= 0;
            const isVisible = stateMatch && textMatch;
            item.style.display = isVisible ? '' : 'none';
            if (isVisible) {
                visibleCount += 1;
            }
        }
        resultsMeta.textContent = 'Showing ' + visibleCount + ' law entries for ' + stateFilter.options[stateFilter.selectedIndex].text + '.';
    ...
    if (stateFilter) {
        stateFilter.addEventListener('change', filterLaws);
    }
    lawSearchInput.addEventListener('input', filterLaws);
    ...
    if (selectedStateFromServer && stateFilter) {
        stateFilter.value = selectedStateFromServer;
    }
    filterLaws();
